...
function parseDelimited(text: string): ImportRow[] {
  ...
  const map = head.map((h) => alias[h.replace(/\s+/g, '')] ?? '');
  return rows.slice(1)
...
function ImportTab() {
  ...
  async function run(dryRun: boolean) {
    ...
    if (!branchId) { toast('Selecciona la sucursal por defecto'); return; }
    setBusy(true); setProgress(0); setReport(null);
    const total: ImportReport = { created: 0, duplicates: 0, errors: [] };
    try {
      for (let i = 0; i < rows.length; i += 200) {
        const chunk = rows.slice(i, i + 200);
        const r = await api.post<ImportReport>('/patients/import', { rows: chunk, branchId, dryRun });
  ...
  return (
    <div className="flex flex-col gap-3.5">
      <div className="rounded-base border border-line bg-card p-4 text-[12.5px] text-muted shadow-card">
        ...
        <div className="mt-2">Columnas: <code className="text-[11.5px]">{IMPORT_HEADER}</code> · Obligatorias: <b>name</b> y <b>phone</b> · <code className="text-[11.5px]">branch</code> acepta el código (e1, e2, e3) y si va vacía usa la sucursal por defecto.</div>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-base border border-line bg-card p-4 shadow-card">
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-xs font-bold text-muted">Sucursal por defecto</span>
