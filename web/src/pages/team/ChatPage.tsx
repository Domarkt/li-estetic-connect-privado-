...
interface Msg { id: string; body: string; senderName: string; senderRole: string; mine: boolean; patient: { id: string; name: string } | null; time: string }
...
export default function ChatPage() {
  ...
  async function send() {
    ...
    try {
      await api.post(`/team-chat/threads/${active}/messages`, { body: draft.trim(), patientId: tagged?.id });
            ...
            <div key={m.id} className="flex" style={{ justifyContent: m.mine ? 'flex-end' : 'flex-start' }}>
              <div className="max-w-[440px]">
                ...
                <div className="rounded-2xl px-3.5 py-2.5 text-[13.5px]" style={m.mine ? { background: 'var(--magenta)', color: '#fff', borderBottomRightRadius: 4 } : { background: '#fff', border: '1px solid var(--line)', borderBottomLeftRadius: 4 }}>
                  {m.patient && <div className={`mb-1 inline-block rounded-md px-2 py-0.5 text-[11px] font-bold ${m.mine ? 'bg-white/25' : 'bg-magenta-soft text-magenta'}`}>🏷 {m.patient.name}</div>}
                  <div>{m.body}</div>
                </div>
