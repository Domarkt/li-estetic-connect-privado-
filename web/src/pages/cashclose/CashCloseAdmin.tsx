...
export default function CashCloseAdmin() {
  ...
  const load = useCallback(() => {
    ...
    api.get<CashCloseAdminView>(`/cashclose/admin?date=${date}${b}`).then(setData).catch(() => {});
  }, [date, activeBranch]);
  useEffect(() => { load(); }, [load]);

  async function reconcile(closeId: string, branchName: string) {
    try { const r = await api.patch<{ message: string }>(`/cashclose/${closeId}/reconcile`, {}); toast(`${branchName}: ${r.message}`); load(); }
  ...
  return (
    <div className="animate-fade">
      <div className="mb-4 flex items-center gap-3">
        ...
        <span className="text-[12px] text-faint">Cuadra cada estética por separado. El conteo lo hace recepción a ciegas.</span>
      </div>
          ...
          return (
            <div key={b.branchId} className="rounded-base border border-line bg-card p-5 shadow-card">
              ...
              <div className="overflow-hidden rounded-[11px] border border-line-2">
                <div className="grid grid-cols-[1.2fr_1fr_1fr_1.2fr] gap-2 bg-navy-soft px-3 py-2 text-[11px] font-bold uppercase text-navy">
                  <div>Método</div><div className="text-right">Esperado</div><div className="text-right">Contado</div><div className="text-right">Diferencia</div>
                </div>
                {b.methods.map((m) => (
                  <div key={m.method} className="grid grid-cols-[1.2fr_1fr_1fr_1.2fr] gap-2 border-t border-line-2 px-3 py-2 text-[12.5px]">
                    <div className="font-semibold">{METHOD_LABEL[m.method]}</div>
                    <div className="text-right">{fmtRD(m.expected)}</div>
                    <div className="text-right">{m.counted == null ? '—' : fmtRD(m.counted)}</div>
                    <div className="text-right font-bold" style={{ color: diffColor(m.diff) }}>{m.diff == null ? '—' : (m.diff === 0 ? '✓' : (m.diff > 0 ? '+' : '') + fmtRD(m.diff))}</div>
                  </div>
                ))}
                <div className="grid grid-cols-[1.2fr_1fr_1fr_1.2fr] gap-2 border-t-2 border-line bg-bg px-3 py-2 text-[13px] font-extrabold">
                  <div>Total</div>
                  <div className="text-right">{fmtRD(b.totalExpected)}</div>
                  <div className="text-right">{b.totalCounted == null ? '—' : fmtRD(b.totalCounted)}</div>
                  <div className="text-right" style={{ color: diffColor(b.totalDiff) }}>{diffLabel(b.totalDiff)}</div>
                </div>
              </div>

              {b.cardVouchers && b.cardVouchers.length > 0 && (
                <div className="mt-2 text-[11.5px] text-muted">Vouchers: {b.cardVouchers.map((v) => fmtRD(v)).join(' · ')}</div>
              )}
              ...
              <div className="mt-4 flex items-center justify-between">
                {b.status === 'CUADRADO'
                  ...
                  : <span className="text-[12.5px] text-muted">{b.status === 'PENDIENTE' ? 'Recepción aún no envía el cierre' : 'Lista para cuadrar'}</span>}
                <button disabled={b.status !== 'ENVIADO' || !b.closeId} onClick={() => b.closeId && reconcile(b.closeId, b.branchName)}
