...
teamRouter.get('/threads', requireStaff, async (req, res) => {
  ...
  const threads = await Promise.all(branches.map(async (b) => {
    const last = await prisma.teamMessage.findFirst({ where: { branchId: b.id }, orderBy: { createdAt: 'desc' } });
    const since = readMap.get(b.id);
    const unread = await prisma.teamMessage.count({
      where: { branchId: b.id, senderId: { not: req.staff!.sub }, ...(since ? { createdAt: { gt: since } } : {}) },
    });
    return {
...
teamRouter.get('/threads/:branchId/messages', requireStaff, async (req, res) => {
  ...
  const messages = await prisma.teamMessage.findMany({ where: { branchId }, orderBy: { createdAt: 'asc' }, take: 200 });
...
const sendSchema = z.object({ body: z.string().trim().min(1), patientId: z.string().optional() });
...
teamRouter.post('/threads/:branchId/messages', requireStaff, async (req, res) => {
  ...
  await prisma.teamMessage.create({
    data: {
      ...
      body: b.body, patientId: b.patientId ?? null, patientName,
    },
  });

  // Aviso por la campana: si escribe el admin → al personal de la sucursal; si escribe la sucursal → al admin.
  const tag = patientName ? ` · sobre ${patientName}` : '';
  if (req.staff!.role === 'ADMIN') {
    await notifyRole('RECEPCIONISTA', { type: 'GENERAL', title: 'Mensaje del administrador', body: `${b.body}${tag}`, link: '/app/chat' }, branchId);
    await notifyRole('ESTETICISTA', { type: 'GENERAL', title: 'Mensaje del administrador', body: `${b.body}${tag}`, link: '/app/chat' }, branchId);
  } else {
    await notifyRole('ADMIN', { type: 'GENERAL', title: `Mensaje de ${req.staff!.name}`, body: `${b.body}${tag}`, link: '/app/chat' });
  }
